...
public class SwaggerDirtyFixConfig {
     ...
     * {@link org.springframework.boot.actuate.endpoint.web.servlet.AbstractWebMvcEndpointHandlerMapping.OperationHandler${symbol_pound}handle(javax.servlet.http.HttpServletRequest, java.util.Map)}
     *
     * This causes 2 issues: - Because the handler method has an @RequestBody
     * annotated 'body' parameter, this parameter appears in all actuator
     * endpoints as body parameter, even for GET and HEAD requests (which cannot
     * have a request body). These endpoints cannot be executed from the Swagger
     * UI page. - If an Actuator endpoint contains path parameters, these are
     * not available as input fields on the Swagger UI page, because no
     *
     * @PathParam annotated arguments are present on the handler method.
     *
